...
<html lang="en">
  ...
  // Saare inputs aur selects ko reset karna
  document.querySelectorAll('input, select').forEach(input => {
    if (input.tagName === 'SELECT') {
      // Dropdown ke liye default value set karna
      input.value = input.options[0].value; // Pehla option by default set hoga
    } else {
      // Input fields ke liye blank karna
      input.value = '';
    }
    input.dispatchEvent(new Event('input')); // Event trigger karna
  });
